...
$cabecera = consultas::get_datos("select * from v_ventas where ven_cod =".$_REQUEST['vven_cod']);
//GENERAR CONSULTA
if (!empty(isset($_REQUEST['opcion']))) {
    switch ($_REQUEST['opcion']) {
        case 1://fecha
            $cabecera = consultas::get_datos("select *from v_ventas where ven_fecha::date between '".$_REQUEST['vdesde']."' and '".$_REQUEST['vhasta']."'");
        
            break;
        case 2://cliente
            $cabecera = consultas::get_datos("select * from v_ventas where cli_cod in(".$_REQUEST['vcliente'].")");
        
            break;
        case 3://articulo
            $cabecera = consultas::get_datos("select * from v_ventas where  in(select ped_cod from detalle_pedventa where art_cod in(".$_REQUEST['varticulo']."))");
            
            break;
        case 4://empleado
            $cabecera = consultas::get_datos("select * from v_ventas where emp_cod in(".$_REQUEST['vempleado'].")");
            
            break;   
    }
}else{
    $cabecera = consultas::get_datos("select * from v_ventas where ped_cod =".$_REQUEST['vped_cod']);
}
