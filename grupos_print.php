...
//COLOR DE TABLA
        ...
        if (!empty(isset($_REQUEST['vdesde'])) && !empty(isset($_REQUEST['vdesde'])) ) {
            if ($_REQUEST['opcion']==1) {
                $cargo = consultas::get_datos("select * from grupos where gru_cod between " .$_REQUEST['vdesde']. " and " .$_REQUEST['vhasta'] ); 
                
            }else {
                
            $cargo = consultas::get_datos("select * from grupos where gru_nombre between '".$_REQUEST['vdesde']."' and '".$_REQUEST['vhasta']."' " );
               } 
            }else{
                
                 $cargo = consultas::get_datos("select * from grupos ");
            }
            
            foreach ($cargo as $cargo) {
            $pdf->Cell(50,5,$cargo['gru_cod'], 1, 0, 'C', 1);
            $pdf->Cell(0,5,$cargo['gru_nombre'], 1, 0, 'L', 1);
            $pdf->Ln();
        }
